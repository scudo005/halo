...
bool action_alert_perform(int actor_handle)
{
  ...
  if (*(short *)(actor + 0x9c) != 0 && *(short *)(actor + 0xa4) == -1) {
    ...
    if (*(short *)(actor + 0xa2) != -1 && set_actor_is_in_veichle(actor_handle) != '\0') {
 ...
 *   identical action_fight_setup twin actor_looking_init_fight / 0x14620).
 ...
 * its timeslice byte is set, evaluate a look target (actor_looking_select_firing_position) and hand
 * the result to the firing-position selector (actor_looking_assign_firing_position).  Returns true
 ...
 * Confirmed actor_looking_select_firing_position pushes (last-to-first, 0x12981..0x1299b): &local_4,
 *   big_buf, &local_8, local_48, state_buf, actor_handle.
 * Confirmed actor_looking_assign_firing_position pushes (last-to-first, 0x129aa..0x129be): local_4,
 *   big_buf, local_8, local_48, EAX (actor_looking_select_firing_position result), actor_handle.
 *   ADD ESP,0x3c at 0x129c4 cleans csmemset (0xc) + both 6-arg calls (0x18
 *   each); the actor_looking_assign_firing_position return value is discarded.
...
bool action_avoid_perform(int actor_handle)
{
  ...
  if (*(char *)(actor + 0x4c) != '\0') {
    ...
    *(short *)(state_buf + 4) = 6;
    result = actor_looking_select_firing_position(actor_handle, state_buf, local_48, &local_8, big_buf,
                          &local_4);
    actor_looking_assign_firing_position(actor_handle, result, local_48, local_8, (unsigned int)big_buf,
                 (char)local_4);
  }
 ...
 *   !=0, actor+0x504(char)==0, and set_actor_is_in_veichle(actor_handle)==0 (PUSH
...
void FUN_00012c30(int actor_handle)
{
  ...
  *(short *)(actor + 0x3fc) = 4;
  ...
  if ((*(short *)(*new_var + 0xa0) == 2 || *(short *)(actor + 0xa0) == 3) &&
      *(char *)(*new_var + 0xa5) != '\0' &&
      (*(char *)(actor + 0x504) == '\0' &&
       set_actor_is_in_veichle(actor_handle) == '\0')) {
