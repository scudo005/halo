...
void actor_move_reset_movement(int actor_handle)
{
  char *actor;
...
void actor_move_build_obstacle_list(int actor_handle)
{
  int handles[2048];
 ...
 *
 * Register args (confirmed from caller actor_move_compute_avoidance @ 0x2c01f-0x2c02b and
 * 0x2c166-0x2c17b):
 ...
 * slot. */
short actor_move_avoidance_ray_cast(float *avoidance_ray, float *ray_origin, int avoidance_data,
 ...
 *
 * Register args (confirmed from caller actor_move_compute_avoidance @ 0x2c4c7-0x2c4cf and
 * 0x2c765-0x2c76d):
...
char actor_move_find_avoidance_sector(float *direction, short count, int records, float *values,
...
/* 0x2b830 — actor_move_choose_facing_vector: choose a desired facing vector.
 *
...
void actor_move_choose_facing_vector(float *facing_basis /* @<ecx> */, char use_3d,
...
void actor_move_facing_to_movement_frame(char use_3d /* @<al> */,
...
void actor_move_compute_avoidance(int actor_handle /* @<ecx> */, float *facing, float *vel_out,
                  float *speed_out)
{
  ...
  *(float *)((char *)state + 0x6044) = 12.0f;
  ...
  actor_move_build_obstacle_list((int)state);
   ...
   * actor_move_avoidance_ray_cast (avoidance_ray@<eax>, ray_origin@<ebx>, avoidance_data@<esi>).
  ...
  table = (int *)0x6327e0; /* per-ray packed-ray records (7 ints/ray) */
  {
    ...
    float collision_t;
    for (n = 9; n != 0; n--) {
      hit_count = (short)actor_move_avoidance_ray_cast((float *)table, ray_origin, (int)state,
   ...
   * directions at avd+0x6418 (stride 3). */
  {
    ...
    for (n = 8; n != 0; n--) {
      ...
      for (k = 0; k < 2; k++) {
        ...
        float po[3];
        float pt;
        probe_hits[k] = (short)actor_move_avoidance_ray_cast(
  ...
  if (*(float *)0x255ca0 < vel_mag) {
    ...
    if (fabsf(dlen) >= *(double *)0x2533d0) {
      ...
      if (*(float *)0x2533c0 < dlen) {
        char ok =
          actor_move_find_avoidance_sector(work, 8, 0x632780, weights, &em_index, &em_value);
        em_scl = *(float *)0x2533c0;
  ...
  if (fabsf(mag) >= *(double *)0x2533d0) {
    ...
    if (*(float *)0x2533c0 < mag) {
      ...
      if (fabsf(mag) >= *(double *)0x2533d0) {
        ...
        work[2] = inv * work[2];
        if (*(float *)0x2533c0 < mag) {
          actor_move_find_avoidance_sector(work, 8, 0x632780, weights, &move_amt, &move_value);
 ...
 * Confirmed: actor_move_facing_to_movement_frame(use_z@al [MOV AL,[EBP+0xc]], mvdir@esi, fdir@edi,
 *            avoid_vec@ebx) at 0x2dbf7, and actor_move_choose_facing_vector(...) at 0x2ddb4
                               ...
                               float *out_vec2, char *out_byte, char *out_bool)
{
  ...
  float *mvdir; /* movement_direction arg to actor_move_facing_to_movement_frame (@esi) */
  float *fdir; /* facing_direction arg to actor_move_facing_to_movement_frame (@edi) */
  ...
  if (move_dir < 0 || move_dir > 3) {
    ...
    if (((actor_t *)actor)->field_505 == '\0') {
      ...
      actor_move_choose_facing_vector((float *)(actor + 0x174), use_z, facing, &move_type,
    ...
    if (want_facing != '\0') {
      ...
      fdir = facing;
    bab0_converge:
      actor_move_facing_to_movement_frame(use_z, mvdir, fdir, avoid_vec);
      move_type = 4;
    }
  }
 ...
 *
 * cdecl, sole arg actor_handle (confirmed: caller actor_activate @ 0x3ed88,
 ...
 *            actor_move_compute_avoidance(actor+0x518 or scaled, &slerp, &weight)@<ecx>.
...
void actor_move_update(int actor_handle)
{
  ...
  float weight; /* [EBP-0x8] (actor_move_compute_avoidance speed out) */
  ...
  *(int *)(actor + 0x5ac) = *(int *)(actor + 0x17c);
  ...
  if (((actor_t *)actor)->field_430 != '\0') {
    ...
    *(int *)(actor + 0x5ec) = 0;
    ...
    actor_move_compute_avoidance(actor_handle, (float *)src, slerp, &weight);
    fade = *(float *)0x2533e8;
...
seed_fallback:
  ...
  *(char *)(actor + 0x508) = crouch;
  ...
  if (actor_is_in_swarm_or_veichle(actor_handle) != '\0' ||
